...
namespace Terminal.Gui
{
    ...
    public class Label : View
    {
        ...
        private readonly List<ustring> lines = new List<ustring>();

        private bool recalcPending = true;

        private ustring text;

        private TextAlignment textAlignment;

        private Attribute textColor = -1;
        ...
        private static Rect CalcRect(int x, int y, ustring s)
        {
            var mw = 0;
            var ml = 1;
        ...
        private static ustring ClipAndJustify(ustring str, int width, TextAlignment talign)
        {
            int slen = str.Length;
            if (slen > width)
                return str[0, width];
            if (talign == TextAlignment.Justified)
            {
                ...
                string[] words = str.ToString().Split(whitespace, StringSplitOptions.RemoveEmptyEntries);
        ...
        private static void Recalc(ustring textStr, List<ustring> lineResult, int width, TextAlignment talign)
        {
            lineResult.Clear();
            if (textStr.IndexOf('\n') == -1)
            {
