...
/// </remarks>
		...
		ustring GetRegion()
		{
			...
			var line = this.model.GetLine(startRow);

			if (startRow == maxrow)
				return this.StringFromRunes(line.GetRange(startCol, endCol));

			var res = this.StringFromRunes(line.GetRange(startCol, line.Count - startCol));
		...
		void ClearRegion()
		{
			...
			var line2 = this.model.GetLine(maxrow);
		...
		public override void Redraw(Rect region)
		{
			...
			for (int row = region.Top; row < bottom; row++) {
				...
				var line = this.model.GetLine(textLine);
		...
		void InsertText(ustring text)
		{
			...
			var rest = line.GetRange(this.CurrentColumn, restCount);
			...
			var last = this.model.GetLine(this.CurrentRow + lines.Count - 1);
		...
		public override bool ProcessKey(KeyEvent kb)
		{
			...
			case Key.Backspace:
				if (this.CurrentColumn > 0) {
					...
					var prevRow = this.model.GetLine(prowIdx);
			...
			case Key.ControlD: // Delete
				...
				if (this.CurrentColumn == currentLine.Count) {
					if (this.CurrentRow + 1 == this.model.Count)
						break;
					var nextLine = this.model.GetLine(this.CurrentRow + 1);
			...
			case Key.ControlK: // kill-to-end
				...
				if (currentLine.Count == 0) {
					...
					var val = this.StringFromRunes(rest);
					if (this.lastWasKill)
						this.AppendClipboard(val);
					else
						this.SetClipboard(val);
			...
			case 'b' + Key.AltMask:
				var newPos = this.WordBackward(this.CurrentColumn, this.CurrentRow);
				if (newPos.HasValue) {
		...
		{
			var line = this.model.GetLine(row);
			if (col + 1 < line.Count) {
				col++;
				rune = line[col];
				return true;
