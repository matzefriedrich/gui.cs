...
namespace Terminal.Gui {
	...
	public class View : Responder, IEnumerable {
		...
		public View MostFocused {
			get {
				if (this.Focused == null)
					return null;
				var most = this.Focused.MostFocused;
				if (most != null)
					return most;
				return this.Focused;
			}
		...
		public Point ScreenToView(int x, int y)
		{
			...
			var parent = this.SuperView.ScreenToView(x, y);
		...
		{
			var bscreen = this.RectToScreen(rect);
		...
		{
			var scrRect = this.RectToScreen(rect);
		...
		public override void Redraw(Rect bounds)
		{
			if (!this.NeedDisplay.IsEmpty) {
				...
				if (this.Title != null && width > 4) {
					...
					Driver.AddRune(' ');
					var str = this.Title.Length > width ? this.Title[0, width - 4] : this.Title;
					Driver.AddStr(str);
					Driver.AddRune(' ');
				}
